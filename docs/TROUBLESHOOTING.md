...
which is not the same thing. 0.7.0's MCP server is the exception, and only
because it is headless: it adds no overlay code and was driven end to end
against a real session.

CI now drives the binary against a real capture on all three platforms
every push, and pins every exit code without a display at all. That covers
everything except the overlay itself, the permission prompts, and
`--target`. Multi-monitor and mixed-DPI are verified on macOS and remain
test-only elsewhere; fractional scaling is unverified everywhere. The
README's platform table is kept honest, and this paragraph says the same
thing it does.
